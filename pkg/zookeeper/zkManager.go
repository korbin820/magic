			...
			case zk.EventNodeDataChanged:
				{
					...
					if strings.Compare(event.Path, "/config/gateway-api1-internal-users") == 0 {
						go func() {
							defer func() {
								if err := recover(); err != nil {
									log.Println(err)
							...
							manager.InternalUsersChangedEvent <- ChangedEvent{
								Path: event.Path,
							}
						}()
					}
...
var cbs = make([]func(zk *ZK) error, 0)

func (m *Manager) Register(callback func(zk *ZK) error) {
	mutex.Lock()
	defer mutex.Unlock()
...
一次性获取zk所有数据
*/
func GetZkCdsData() map[string]*EDS {

	NewManager()

	mapResult := make(map[string]*EDS)
