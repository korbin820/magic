...
var TRACE_HEADERS_TO_PROPAGATE = []string{
	"X-Ot-Span-Context",
	"X-Request-Id",
...
func do(ctx context.Context, method string, url string, contenType string, body io.Reader) ([]byte, error) {
	...
	if ctx.Value("headers") != nil {
		header := ctx.Value("headers").(http.Header)

		for _, v := range TRACE_HEADERS_TO_PROPAGATE {
			if header != nil && header[v] != nil {
				if len(header[v]) > 0 {
					req.Header.Set(v, header[v][0])
				}
			}
		}
	}
	//fmt.Println(util.StructToJson(req.Header))

	if err != nil {
		panic(err)
	}
	//req = req.WithContext(traceCtx)
	reqCtx, cncl := context.WithTimeout(ctx, time.Second*5)
	defer cncl()
